  ...
  describe('preprocessor function', function() {
    ...
    describe('when it finishes cleanly', function() {
      ...
      it('specifies the entry file', function() {
        return this.run().then(() => {
          expect(browserify.lastCall.args[0].entries[0]).to.equal(
            this.file.filePath
          )
        })
      })
      ...
      it('specifies default extensions if none provided', function() {
        return this.run().then(() => {
          expect(browserify.lastCall.args[0].extensions).to.eql([
            '.js',
            '.jsx',
            '.coffee',
            '.cjsx',
    ...
    describe('when it errors', function() {
      ...
      it('does not trigger unhandled rejection when bundle errors after update', function(
        done
      ) {
        const handler = sandbox.spy()
        process.on('unhandledRejection', handler)
        this.createWriteStreamApi.on
          .withArgs('finish')
          .onFirstCall()
          .yields()
        ...
        this.run().then(() => {
          streamApi.on
            .withArgs('error')
            .yieldsAsync(new Error('bundle error'))
            .returns({ pipe() {} })
